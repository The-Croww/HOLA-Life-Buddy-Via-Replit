...
const Audio = {
  ...
  Sound: {
    createAsync: async (_src: object, _opts?: object) => ({
      sound: {
        stopAsync: async () => {},
        unloadAsync: async () => {},
        setOnPlaybackStatusUpdate: (_cb: (s: { isLoaded: boolean }) => void) => {},
      },
    }),
  },
} as const;
...
export default function MindfulnessScreen() {
  ...
  return (
    <View style={styles.container}>
          ...
          <>
            ...
            <View style={styles.toolsGrid}>
              {TOOLS.map((tool) => {
                const onPress = () => {
                  Haptics.selectionAsync();
                  if (tool.id === "worry") setShowWorry(true);
                  else if (tool.id === "gratitude") setShowGratitude(true);
                  else if (tool.id === "reframe") setShowReframe(true);
                  else if (tool.id === "grounding") setShowGrounding(true);
                };
                const isInteractive = ["worry", "gratitude", "reframe", "grounding"].includes(tool.id);
                return (
                  <TouchableOpacity
                    key={tool.id}
                    style={[styles.toolCard, isInteractive && { borderColor: tool.color + "55", borderWidth: 1.5 }]}
                    activeOpacity={0.75}
                    onPress={onPress}
                  >
                    ...
                    {isInteractive ? (
                      <View style={[styles.comingSoonBadge, { backgroundColor: tool.color + "18" }]}>
                        <Text style={[styles.comingSoonText, { color: tool.color }]}>Open →</Text>
                      </View>
                    ) : (
      ...
      <WorryBoxModal visible={showWorry} onClose={() => setShowWorry(false)} />
      <GratitudeJarModal visible={showGratitude} onClose={() => setShowGratitude(false)} />
      <ThoughtReframeModal visible={showReframe} onClose={() => setShowReframe(false)} />
      <GroundingModal visible={showGrounding} onClose={() => {
